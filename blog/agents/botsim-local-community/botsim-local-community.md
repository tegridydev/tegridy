+++
title = "BotSim: A Local AI Agent Community Simulator"
date = "2026"
description = "Explore scheduling, memory and restart behaviour in a small local agent community, with deterministic fixture checks kept separate from live model behaviour."
draft = false
...
See [app.py](app.py) or the [module README](README.md).
...
The live Ollama adapter hasn't been exercised against an installed model in the reported evaluation. Scheduler comparisons, token accounting, relationship metrics and multi process workers are still outside the first build.
