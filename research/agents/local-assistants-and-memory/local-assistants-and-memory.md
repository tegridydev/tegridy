+++
title = "Local AI Memory: Retrieval and Temporal Validity"
date = "2026"
description = "Compare current and historical assistant memory using ownership, validity and recording time, with a fixed word budget and bounded synthetic results."
draft = false
...
| recent · historical · recall (%) | 100 | 0 |
| recent · known then · recall (%) | 100 | 0 |
| relevant · current · recall (%) | 100 | 0 |
| relevant · historical · recall (%) | 100 | 0 |
| relevant · known then · recall (%) | 100 | 0 |
...
**Question:** does source linked, query relevant retrieval improve multi session recall over recency only context under an equal context budget? The current fixture counts whitespace separated words, not model tokens. A model comparison would need token accounting as a separate check.

The earlier retrieval fixture uses 30 synthetic histories containing stable facts, corrections, irrelevant memories, historical questions and separate owners that can share the same display name. Ten histories are for tuning and twenty are held out. A deterministic recorder captures exactly what each policy placed into context.
...
See [retrieval.py](retrieval.py); the [module README](README.md) describes usage and dependencies.
